...
} from '~src/units/ContactFieldInput'
...
const backupNameField = nameFieldMap({
  id: '',
  firstName: '',
  middleName: '',
  lastName: '',
...
const backupEmailField = emailFieldMap({
  id: '',
  email: '',
  title: '',
  priority: 100,
...
const backupPhoneField = phoneFieldMap({
  id: '',
  number: '',
  title: '',
  priority: 100,
...
const backupDateField = dateFieldMap({
  id: '',
  year: 0,
  month: 0,
  day: 0,
...
const backupAddressField = addressFieldMap({
  id: '',
  firstLine: '',
  secondLine: '',
  title: '',
...
const backupOtherField = otherFieldMap({
  id: '',
  content: '',
  title: '',
  priority: 100,
...
const specificFieldToInputField =
  (fieldType: FieldType) =>
  (
    specificField:
      | NameField
...
const ContactProfile: React.FC<Props> = React.memo(({ contactId }) => {
  ...
  const handleSplitWaiver = useCallback(async () => {
    cancelSplitWaiver()
  ...
  const {
    value: editable,
    setTrue: toggleOnEditable,
    setFalse: toggleOffEditable,
  } = useBoolean(false)
  ...
  const handleDeleteTag = useCallback(
    (tag: string) => () => removeTag(tag),
    [removeTag],
  )

  const handleUpdateContactGender = useCallback(
    (value: 'Male' | 'Female' | 'Other') =>
      ['Male', 'Female', 'Other'].includes(value) && updateContactGender(value),
    [updateContactGender],
  )
  ...
  const handleAddField = useCallback(
    async (
      ...
      specificFieldToInputField(fieldType)(
        await addField({ ...obj, fieldType, priority }),
      ),
    [addField],
  )
  ...
  const handleUpdateField = useCallback(
    async (
      ...
      specificFieldToInputField(fieldType)(
        await updateField({ ...updateObj, id, fieldType, priority }),
      ),
    [updateField],
  )
  ...
  const handleChangeFieldPriority = useCallback(
    async (fieldType: FieldType, id: string, priority: number) =>
      specificFieldToInputField(fieldType)(
        await updateField({ id, fieldType, priority } as any),
      ),
    [updateField],
  )
  ...
  const infoFields = useMemo<
    Record<
      'names' | 'emails' | 'phones' | 'dates' | 'addresses' | 'others',
      FieldValue[]
    >
  >(
    () => ({
  ...
  const toggleSplitter = useCallback(async () => {
    if (!toSplitWaiver.id) {
  ...
  const renderTags = (isFloating: boolean) =>
    !editable && (
      <Hidden lgDown={isFloating} xlUp={!isFloating}>
        <div
          className={
            isFloating ? classes.floatTagsWrapper : classes.blockTagsWrapper
          }
        >
          <div className={classes.tagsBar}>
            <Input
              ...
              disableUnderline
              startAdornment={
                <Icon name={ICONS.Tag} className={classes.addTagIcon} />
              }
        ...
        <SplitWaiverThemeProvider>
          ...
          >
            ...
            <div className={classes.dialogButtonZone}>
              ...
              <Button color="primary" onClick={handleSplitWaiver}>
                Split
              </Button>
            </div>
          </Dialog>
