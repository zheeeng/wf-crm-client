...
export interface CreateFormOption {
  title?: string
  tip?: string
  fields: (TextField | CombinedTextField | EnumTextField | CountryField)[]
  okTextWatch?: string
...
export interface Props {
  ...
  onOk?: (o: Record<string, unknown>) => Promise<any>
  option?: CreateFormOption
  discardText?: string
}

const CreateForm: React.FC<Props> = React.memo(
  ({ option, open, onClose, onOk, discardText }) => {
    ...
    const [watchValue, setWatchValue] = useState(
      fieldValues.current[okTextWatch] ?? '',
    )
      ...
      () =>
        option
          ? option.fields
              ...
              }, [] as { name: string; required: boolean }[])
              .filter((p) => p.required)
              .map((p) => p.name)
          : [],
      [option],
    ...
    const validators = useMemo(
      () =>
        option
          ? option.fields.reduce((acc, field) => {
              switch (field.type) {
                case 'text':
                case 'enumText':
                  return field.validator
                    ? [...acc, { name: field.name, validator: field.validator }]
                    : acc
                case 'combinedText':
                  return [
                    ...acc,
    ...
    const handleCreateInfoSubmit = useCallback(
      async (e: React.SyntheticEvent) => {
        ...
        const invalidFieldNames = validators
          .filter((v) => values[v.name] && !v.validator(values[v.name]))
          .map((v) => v.name)
      ...
      <>
        ...
        >
          ...
          {fields.map((field, index) =>
            field.type === 'text' ? (
              <BasicFormInput
                ...
                placeholder={
                  field.label +
                  (toWarnFields.includes(field.name) &&
                  validators.find((v) => v.name === field.name)
                    ? ' (invalid format)'
                    : '')
                }
              ...
              <div key={field.keyName} className={classes.combinedFormRow}>
          ...
          <div className={classes.dialogButtonZone}>
            ...
            <Button
              disabled={
                typeof okColor === 'function' &&
                okColor(watchValue) === 'default'
                  ? true
                  : false
              }
              color={
                typeof okColor === 'function' ? okColor(watchValue) : okColor
              }
